...
def generate_clicker_parts(mesh: trimesh.Trimesh, slice_z: float, cfg: dict):
    ...
    center_support = make_cylinder(
        ...
        [center[0], center[1], center_support_z],
    )
...
def add_wire_box(
    ...
    """Draw a true-size wireframe preview box for a cavity/cut volume."""
    cx, cy, cz = center
...
def add_cavity_preview_boxes(fig: go.Figure, mesh: trimesh.Trimesh, slice_z: float, cfg: dict):
    """Show the actual top and bottom cavity cut volumes, not a generic reference cube."""
    center = mesh.bounds.mean(axis=0)

    top_cavity_z = slice_z + cfg["housing_depth"] / 2
    add_wire_box(
        fig,
        center=[center[0], center[1], top_cavity_z],
    ...
    add_wire_box(
        fig,
        center=[center[0], center[1], bottom_cavity_z],
...
def make_preview_figure(mesh: trimesh.Trimesh, slice_z: float, cfg: dict, generated=False, top_mesh=None, bottom_mesh=None):
    fig = go.Figure()
    ...
    if generated and top_mesh is not None and bottom_mesh is not None:
        ...
        # Separate along the Y axis so the mating faces are easier to inspect.
