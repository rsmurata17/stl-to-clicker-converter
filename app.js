...
function cleanGeometry(geometry) {
  ...
  // three-bvh-csg is strict: all brushes must have the same attributes.
  // STL geometry usually has position/normal only, while Three.js primitives
  // often include uv. Remove every extra attribute so source/cutters match.
...
function makeBrushBox(x, y, z, center) {
  const geometry = cleanGeometry(new THREE.BoxGeometry(x, y, z));
...
function makeBrushCylinder(diameter, height, center) {
  const geometry = cleanGeometry(new THREE.CylinderGeometry(diameter / 2, diameter / 2, height, 64, 1, false));
