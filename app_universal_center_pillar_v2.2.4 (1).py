...
with st.sidebar:
    ...
    if bottom_space < bottom_needed:
        st.warning("Slice may be too low for the full bottom cavity.")
    ...
    st.caption(
        f"Top clearance: {top_space:.2f} / {top_needed:.2f} mm"
    )
    st.caption(
        f"Bottom clearance: {bottom_space:.2f} / {bottom_needed:.2f} mm"
    )

    if top_space < top_needed:
        st.warning(
            f"⚠️ Slice is {top_needed - top_space:.2f} mm too high. The top cavity may be clipped."
        )

    if bottom_space < bottom_needed:
        st.warning(
            f"⚠️ Slice is {bottom_needed - bottom_space:.2f} mm too low. The bottom cavity may be clipped."
        )
